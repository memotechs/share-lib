...
import { NotFoundErrorException } from './not-found-error.exception';

describe('NotFoundErrorException', () => {
  it('should extend BadRequestException', () => {
    const options = {}; // Create options object as needed
    const error = new NotFoundErrorException(options);

    expect(error instanceof BadRequestException).toBe(true);
  });

  it('should set options correctly', () => {
    const options = { message: 'Not found' }; // Create options object as needed
    const error = new NotFoundErrorException(options);
